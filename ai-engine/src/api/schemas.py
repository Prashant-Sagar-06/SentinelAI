...
class RemediationResponse(BaseModel):
    """
    Nested schema for remediation guidance.
    
    Provides structured guidance on how to fix the identified root cause.
    This is generated by the deterministic remediation engine and includes
    actionable fix steps, priority level, and time estimates.
    """
    issue_category: str = Field(..., description="Category of the issue (e.g., database_connection_error)")
    description: str = Field(..., description="Professional explanation of the issue and impact")
    fix_steps: List[str] = Field(..., description="Ordered list of actionable remediation steps")
    priority: str = Field(..., description="Priority level (LOW, MEDIUM, HIGH, CRITICAL)")
    estimated_resolution_time: str = Field(..., description="Estimated time to resolve (e.g., '5-15 minutes')")
    confidence_score: float = Field(..., description="Confidence in this remediation recommendation (0-1)")
...
class RootCauseResponse(BaseModel):
    ...
    Includes both the root cause analysis and recommended remediation steps.
    """
    ...
    remediation: Optional[RemediationResponse] = Field(None, description="Recommended remediation steps and guidance")
